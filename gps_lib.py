...
class GPS:
    def __init__(self,txPin,rxPin,baudRate,id = 0):
        self.uart = machine.UART(0, baudrate=baudRate, tx = machine.Pin(txPin), rx = machine.Pin(rxPin))
    ...
    def read_data(self):
        while self.uart.any()>0:
            ...
            if sentence.startswith('$GPGGA'):
                ...
                if len(check_sentence)>10:
                    lat,lat_dir,lon,lon_dir,alt= self.GPGGA(sentence)
                    data = "%0.6f,%s,%0.6f,%s,%0.2f" % (lat,lat_dir,lon,lon_dir,alt)
                    break
            time.sleep(0.1)
        return data
